...
class Controller {
  ...
  /**
   * La instancia de la clase QuetzalHttp
   *
   * @var QuetzalHttp
   */
  protected $http                  = null;
  ...
  private function isRegularController()
  {
    ...
    $this->addToData('cart'       , QuetzalCartHandler::get());

    QuetzalHookManager::runHook('is_regular_controller', $this);
  }
  ...
  private function isEndpointController()
  {
    ...
    try {
      ...
      QuetzalHookManager::runHook('before_process_request_endpoint', $this->http);
      $this->http->process();
      $this->request = $this->http->get_request();
      $this->data    = $this->request['data'];
      $this->files   = $this->request['files'];
      QuetzalHookManager::runHook('is_endpoint_controller', $this);
    } catch (QuetzalHttpException $e) {
      http_response_code($e->getStatusCode());
      json_output(json_build($e->getStatusCode(), [], $e->getMessage()));
