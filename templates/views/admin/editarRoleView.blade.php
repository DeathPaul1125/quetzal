...
<div class="max-w-4xl space-y-6">
  ...
  <div class="flex items-center justify-between">
    <a href="admin/roles" class="text-sm text-slate-500 hover:text-slate-800 inline-flex items-center gap-1">
      <i class="ri-arrow-left-line"></i> Volver a Roles
    </a>
    @if($isProtected)
      <span class="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-amber-50 text-amber-700 text-xs font-medium">
        <i class="ri-lock-line"></i> Role protegido (solo permisos editables)
      </span>
    @endif
  </div>

  <form method="post" action="admin/post_role_editar" class="space-y-6">
    @csrf
    ...
    <div class="bg-white rounded-xl border border-slate-200 p-6">
      <div class="flex items-center justify-between mb-4">
        ...
        <div class="flex items-center gap-2 text-xs">
          ...
          <button type="button" data-q-perms-select="none" class="text-slate-500 hover:underline">Deseleccionar todos</button>
        </div>
      </div>

      @if(empty($allPerms))
        <p class="text-sm text-slate-500 text-center py-6">
          No hay permisos registrados. <a href="admin/permisos" class="text-primary hover:underline">Crea uno primero</a>.
        </p>
      @else
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-2.5" id="q-perms-grid">
          @foreach($allPerms as $p)
            @php $checked = in_array($p['slug'], $rolePerms, true); @endphp
            <label class="flex items-start gap-3 p-3 rounded-lg border border-slate-200 hover:border-primary hover:bg-slate-50 transition cursor-pointer {{ $checked ? 'border-primary bg-amber-50/40' : '' }}">
