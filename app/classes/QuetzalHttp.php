 ...
 * de Quetzal framework.
 * 
 * Esta clase implementa las funcionalidades anteriores de Quetzal ajaxController
 * en su versión 1.1.3
 * 
 * @version 1.0.2
 * 
...
class QuetzalHttp
{
  ...
  function __construct(array $options = [])
  {
    ...
    $this->authenticate = isset($options['authenticate']) ? $options['authenticate'] : quetzal_api_authentication();
  ...
  private function validateRequestVerb()
  {
    if(!in_array(strtoupper($this->r_type), $this->accepted_verbs)) {
      throw new QuetzalHttpException(
  ...
  /**
   * Compara la api key privada de esta instancia de Quetzal framework
  ...
  public function authenticate_request()
  {
    ...
    $api_key = get_quetzal_api_private_key();

    if (strcmp($api_key, $this->private_key) !== 0) {
      throw new QuetzalHttpException(get_quetzal_message(0), 403);
    }
  ...
  private function validate_csrf()
  {
    ...
    if (empty($this->csrf)) {
      throw new QuetzalHttpException('CSRF token requerido.', 401);
    }

    if (!Csrf::validate($this->csrf)) {
      throw new QuetzalHttpException('CSRF inválido.', 401);
    }
  }
