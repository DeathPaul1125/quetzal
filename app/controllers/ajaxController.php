...
class ajaxController extends Controller implements ControllerInterface
{
  ...
  /**
   * Función de pruebas Quetzal framework
   * @since 1.1.4
   *
   * @return void
   */
  ...
  function test_add_post()
  {
    try {
      ...
      if (!Auth::validate()) {
        throw new Exception(get_quetzal_message('auth'));
      }
      ...
      if (!$id = Model::add('pruebas', $data)) {
        throw new Exception(get_quetzal_message('not_added'));
      }

      $post = Model::list('pruebas', ['id' => $id], 1);

      json_output(json_build(201, $post, get_quetzal_message('added')));
    } catch (Exception $e) {
      json_output(json_build(400, null, $e->getMessage()));
    }
  }
  ...
  function test_delete_post()
  {
    try {
      ...
      if (!Model::remove('pruebas', ['id' => $post['id']])) {
        throw new Exception(get_quetzal_message('not_deleted'));
      }
