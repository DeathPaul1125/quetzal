$(document).ready(function () {
  ...
  function init_tooltips() {
    if (['bs', 'bs5', 'bs_lumen', 'bs_lux', 'bs_litera', 'bs_vapor', 'bs_zephyr'].includes(Quetzal.css_framework) != true) return;
  ...
  function init_toastr_setup() {
    if (Quetzal.toastr === false) return;
...
/**
 * Mostrar Quetzal object en entorno de desarrollo
 */
function init_quetzal_greeting() {
  console.log('////////// Bienvenido a Quetzal Versión ' + Quetzal.quetzal_version + ' //////////');
  if (Quetzal?.is_local == true) {
    console.log(Quetzal);
  }
}
...
async function test_api() {
  ...
  const res = await fetch(Quetzal.url + 'api/posts', {
    headers: { 'Authorization': `Bearer ${Quetzal.private_key}` },
    method: 'GET'
  }).then(res => res.json());
