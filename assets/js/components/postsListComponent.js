  ...
  methods: {
    async addPost() {
      ...
      const res = await fetch(Quetzal.url + `api/posts`, {
        headers: { 'Authorization': `Bearer ${Quetzal.private_key}`, 'Content-Type': 'application/json' },
        method: 'POST',
    ...
    async updatePost() {
      ...
      const res = await fetch(Quetzal.url + `api/posts/${this.postEdit.id}`, {
        headers: { 'Authorization': `Bearer ${Quetzal.private_key}`, 'Content-Type': 'application/json' },
        method: 'PUT',
        body: JSON.stringify(body)
      }).then(res => res.json());
    ...
    async removePost(id) {
      ...
      // Petición de borrado
      const res = await fetch(Quetzal.url + `api/posts/${this.postId}`, {
    ...
    <ul class="list-group">
      <li class="list-group-item" v-for="(post, index) in posts" :key="post.id">
        ...
        <div className="btn-group float-end">
          <a :href="'quetzal/test-component/' + post.id" className="btn btn-sm btn-primary"><span className="fas fa-eye"></span></a>
