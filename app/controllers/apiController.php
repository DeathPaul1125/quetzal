...
class apiController extends Controller implements ControllerInterface 
{
  ...
  /**
   * Versión de la API actual de Quetzal
   *
   * @var string
   */
  ...
  private function post_posts()
  {
    if (!check_posted_data(['nombre','titulo','contenido'], $this->data)) {
      throw new QuetzalJsonException('Parámetros faltantes en la petición.', 400, 'missing_params');
    }

    // Validar el tipo de parámetro solicitado
    if (!is_string($this->data['nombre'])) {
      throw new QuetzalJsonException('Parámetro nombre debe ser string.', 400, 'param_error');
    }

    if (strlen($this->data['nombre']) < 5) {
      throw new QuetzalJsonException('Parámetro nombre debe ser mayor a 5 caracteres.', 400, 'param_error');
    }
    ...
    if (!$id = Model::add('pruebas', $data)) {
      throw new QuetzalJsonException(get_quetzal_message('not_added'), 400, 'db_error');
    }
  ...
  private function put_post($id)
  {
    try {
      ...
      if (!$post = Model::list('pruebas', ['id' => $id], 1)) {
        throw new Exception(get_quetzal_message('not_found'));
      }
      ...
      if (!Model::update('pruebas', ['id' => $id], $data)) {
        throw new Exception(get_quetzal_message('not_updated'));
      }

      $post = Model::list('pruebas', ['id' => $id], 1);
      
      json_output(json_build(200, $post, get_quetzal_message('updated')));
  ...
  {
    if (!$post = Model::list('pruebas', ['id' => $id], 1)) {
      throw new QuetzalJsonException(get_quetzal_message('not_found'), 400, 'not_found');
    }

    if (!Model::remove('pruebas', ['id' => $id], 1)) {
      throw new QuetzalJsonException(get_quetzal_message('not_deleted'), 400, 'not_deleted');
    }
