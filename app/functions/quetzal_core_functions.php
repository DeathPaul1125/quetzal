...
/**
 * Regresa el nombre del framework Quetzal
 *
 * @return string
 */
function get_quetzal_name()
{
	return QUETZAL_NAME;
}
 ...
 * @return string
 */
function get_quetzal_version()
{
	return QUETZAL_VERSION;
}

/**
 * Regresa la versión actual del core de Quetzal
 *
 * @return string
 */
function get_core_version()
{
	$file = 'quetzal_core_version.php';
...
function build_url(string $url, array $params = [], bool $redirection = true, bool $csrf = true)
{
	...
	if (!isset($raw_url['host'])) {
		$url     = URL . $url; // se concatena el URL por defecto de Quetzal si no existe un host ya en la URL
		$raw_url = parse_url($url);
	}
	...
	// ejemplo http://localhost:8848/Quetzal/test
...
function logger($message, $type = 'debug', $output = false)
{
	...
	$filename = is_local() ? "dev_log.log" : "quetzal_log.log";
	if (!$fh = fopen(LOGS . $filename, 'a')) {
		error_log(sprintf('Can not open this file on %s', LOGS . 'quetzal_log.log'));
		return false;
	}
...
/**
 * Función para cargar el url de nuestro asset logotipo de Quetzal framework
 *
 * @return string
 */
function get_quetzal_logo()
{
	$default_logo = QUETZAL_LOGO;
...
{
	global $Quetzal_User; // Información persistente del usuario
	 ...
	 * con este ajuste usando el objeto global Quetzal_User, cargamos la información que es registrada cada que carga la página.
	 * @version 1.5.5
	 */
	...
	return $key === null ? $Quetzal_User : (isset($Quetzal_User[$key]) ? $Quetzal_User[$key] : false);
}
...
function load_styles()
{
	...
	if (empty($Quetzal_Styles)) {
		return $output;
	}

	// Iterar sobre cada elemento registrado
	foreach (json_decode(json_encode($Quetzal_Styles)) as $css) {
		if ($css->comment) {
			$output .= '<!-- ' . $css->comment . ' -->' . "\n";
		}
...
function load_scripts()
{
	...
	if (empty($Quetzal_Scripts)) {
		return $output;
	}

	// Itera sobre todos los elementos registrados
	foreach (json_decode(json_encode($Quetzal_Scripts)) as $js) {
		if ($js->comment) {
			$output .= '<!-- ' . $js->comment . ' -->' . "\n";
		}
...
/**
 * Registar un nuevo valor para el objeto Quetzal
 ...
 * @return bool
 */
function register_to_quetzal_obj($key, $value)
{
	global $Quetzal_Object;
	...
	if (is_array($value) || is_object($value)) {
		$Quetzal_Object[$key] = $value;
	} else {
		$Quetzal_Object[$key] = clean($value);
	}
...
/**
 * Carga el objeto Quetzal registrado y todos sus valores
 * por defecto y personalizados
 *
 * @return string
 */
function load_quetzal_obj()
{
	global $Quetzal_Object;
	$output = '';

	if (empty($Quetzal_Object)) {
		return $output;
	}

	$output = '<script>var Quetzal = %s;</script>';

	return sprintf($output, json_encode_utf8($Quetzal_Object));
}

/**
 * Registra los parámetro por defecto de Quetzal
 *
 * @return array
 */
function quetzal_obj_default_config()
{
	$options =
		[
			'sitename'      => get_sitename(),
			'version'       => get_version(),
			'quetzal_name'      => get_quetzal_name(),
			'quetzal_version'   => get_quetzal_version(),
			'csrf'          => CSRF_TOKEN,
			'url'           => URL,
			'cur_page'      => CUR_PAGE,
			'is_local'      => IS_LOCAL,
			...
			'lightbox'      => LIGHTBOX,
			'vuejs'         => VUEJS,
			'public_key'    => get_quetzal_api_public_key(),
			'private_key'   => get_quetzal_api_private_key()
		];
 ...
 * en la carpeta de imágenes de Quetzal framework
 *
...
/**
 * Muestra toda la información actual de Quetzal
 * y sus variables de configuración
 *
 * @return mixed
 */
function get_quetzal_info()
{
	$db   = Db::connect();
	$data =
		[
			'Título'               => sprintf('Quetzal %s', get_quetzal_version()),
			'Versión Quetzal'      => get_quetzal_version(),
	...
	return get_module('quetzal/info', $data);
}
 ...
 * un die de sitio con Quetzal framework
...
function new_cookie($name, $value, $lifetime = null, $path = '', $domain = '')
{
	...
	$lifetime = defined('QUETZAL_COOKIE_LIFETIME') && $lifetime === null ? QUETZAL_COOKIE_LIFETIME : (!is_integer($lifetime) ? $default : $lifetime);
 ...
 * su uso en el sistema de Quetzal
 *
 * @return array
 */
function get_quetzal_default_messages()
{
	$messages =
		[
 ...
 * array de Quetzal_Messages del sistema Quetzal framework
 *
 * @return array
 */
function get_all_quetzal_messages()
{
	global $Quetzal_Messages;

	return $Quetzal_Messages;
}

/**
 * Registra un nuevo mensaje al array
 * de mensajes para su uso en Quetzal framework
 *
 * @param string $code
 * @param string $message
 * @return bool
 */
function register_quetzal_custom_message($code, $message)
{
	global $Quetzal_Messages;

	try {
		if (isset($Quetzal_Messages[$code])) {
			throw new Exception(sprintf('Ya existe el código de mensaje %s.', $code));
		}

		$Quetzal_Messages[$code] = $message;
...
/**
 * Carga un mensaje de Quetzal framework existente
 * en el array de la global Quetzal_Messages
 * 
 * OPCIONES ACTUALES
 * '0'           => 'Acceso no autorizado.'
 * '1'           => 'Acción no autorizada.'
 ...
 * @return string
 */
function get_quetzal_message($code)
{
	global $Quetzal_Messages;

	$code = (string) $code;

	return isset($Quetzal_Messages[$code]) ? $Quetzal_Messages[$code] : '';
}
 ...
 * @return string
 */
function get_quetzal_api_public_key()
{
	$name = 'API_PUBLIC_KEY';
	if (!defined($name)) {
 ...
 * @return string
 */
function get_quetzal_api_private_key()
{
 ...
 * recursos de la API de Quetzal framework y la instancia actual
 *
 * @return bool
 */
function quetzal_api_authentication()
{
	$name = 'API_AUTH';
	if (!defined($name)) {
		throw new Exception(sprintf('La constante %s no existe o no se ha definido en el sistema y es requerida para esta función.', $name));
...
function set_page_og_meta_tags($title = null, $description = null, $image = null, $url = null, $type = 'article')
{
	...
	$OG_Image       = $image === null ? IMAGES . 'quetzal.svg'  : $image;
