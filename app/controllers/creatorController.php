...
class creatorController extends Controller implements ControllerInterface 
{
  ...
  function post_controller()
  {
    try {
      ...
      $name     = sanitize_input($_POST['filename']);
      ...
      if (!is_string($name) || !is_alphanumeric($name)) {
        throw new Exception(sprintf('Ingresa un nombre de controlador válido por favor.', $name));
      }
  
      // Validar longitud del nombre
      if (empty($name)) {
        throw new Exception(sprintf('Ingresa un nombre de controlador válido por favor, <b>%s</b> es demasiado corto.', $name));
      }
      ...
      if (is_file(CONTROLLERS . $filename . $keyword . '.php')) {
        throw new Exception(sprintf('Ya existe el controlador %s.', $filename . $keyword));
      }
  
      // Validar la existencia de la plantilla.txt para crear el controlador
      if (!is_file($template)) {
        throw new Exception(sprintf('No existe la plantilla %s.', $template));
      }
  
      // Cargar contenido del archivo
      $php = @file_get_contents($template);
      $php = str_replace('[[REPLACE]]', $filename, $php);
      $php = str_replace('[[ENGINE]]', 'blade', $php);
  
      // Generar el archivo del controlador
      if (file_put_contents(CONTROLLERS . $filename . $keyword . '.php', $php) === false) {
        throw new Exception(sprintf('Ocurrió un problema al crear el controlador %s.', $template));
      }
      ...
      // Generar la vista solo si así se solicita (siempre Blade)
      if ($g_vista === true) {
        $viewTemplate = MODULES . 'quetzal' . DS . 'viewBladeTemplate.txt';
        ...
        if (!is_file($viewTemplate)) {
          ...
          @file_put_contents(VIEWS . $filename . DS . 'indexView.blade.php', $html);
        }
      }
  ...
  function post_model()
  {
    try {
      ...
      // Validar nombre de archivo
      $name     = sanitize_input($_POST["filename"]);
      $table    = sanitize_input($_POST["tabla"]);
      $scheme   = sanitize_input($_POST["esquema"]);
      $scheme_s = '';
      $filename = strtolower($name);
      $filename = str_replace([' ','-'], '_', $filename);
      $filename = str_replace('.php', '', $filename);
      $keyword  = 'Model';
      $template = MODULES . 'quetzal' . DS . 'modelTemplate.txt';

      // Validar longitud del nombre
      if (empty($name)) {
        throw new Exception(sprintf('Ingresa un nombre de modelo válido por favor, <b>%s</b> es demasiado corto.', $name));
      }

      // Validar longitud del nombre
      if (strlen($table) < 3 && !empty($table)) {
        throw new Exception(sprintf('Ingresa un nombre de tabla válido por favor, <b>%s</b> es demasiado corto.', $table));
      }

      // Validar la existencia de un duplicado
      if (is_file(MODELS.$filename.$keyword.'.php')) {
        throw new Exception(sprintf('Ya existe el modelo %s.', $filename.$keyword));
      }
      ...
      if (file_put_contents(MODELS . $filename . $keyword . '.php', $php) === false) {
        throw new Exception(sprintf('Ocurrió un problema al crear el modelo %s.', $template));
      }
  ...
  function post_view()
  {
    try {
      ...
      $keyword    = 'View';
      $template   = MODULES . 'quetzal' . DS . 'viewBladeTemplate.txt';
      $filename   = sprintf('%s%s.blade.php', $viewName, $keyword);
